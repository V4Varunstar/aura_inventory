...
import { addInward } from '../../services/firebaseService';
import { useToast } from '../../context/ToastContext';
import { useCompany } from '../../context/CompanyContext';

interface BulkInwardUploadProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}
...
const BulkInwardUpload: React.FC<BulkInwardUploadProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const downloadTemplate = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Inward Template');
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const jsonData: InwardRow[] = XLSX.utils.sheet_to_json(worksheet);
      ...
      jsonData.forEach((row, index) => {
        ...
        if (!row['Invoice/Ref No']) validationErrors.push(`Row ${rowNum}: Invoice/Ref No is required`);
        ...
        if (!row['Mfg Date']) validationErrors.push(`Row ${rowNum}: Mfg Date is required`);
        if (!row['Exp Date']) validationErrors.push(`Row ${rowNum}: Exp Date is required`);
        if (!row['Cost Price'] || row['Cost Price'] < 0) validationErrors.push(`Row ${rowNum}: Valid cost price is required`);
        if (!row['Warehouse']) validationErrors.push(`Row ${rowNum}: Warehouse is required`);
        if (!row['Source']) validationErrors.push(`Row ${rowNum}: Source is required`);

        if (validationErrors.length === 0 || validationErrors.filter(e => e.startsWith(`Row ${rowNum}:`)).length === 0) {
          validRecords.push({
            documentNo: row['Invoice/Ref No'],
            sku: row['SKU'],
            ean: row['EAN'] || '',
            quantity: Number(row['Quantity']),
            batchNo: row['Batch No'] || '',
            mfgDate: parseDate(row['Mfg Date']),
            expDate: parseDate(row['Exp Date']),
            costPrice: Number(row['Cost Price']),
      ...
      if (successCount > 0) {
        addToast(`Successfully uploaded ${successCount} inward records!`, 'success');
        onSuccess();
        onClose();
      }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Bulk Inward Upload">
          ...
          <div className="flex items-start">
            ...
            <div className="text-sm text-blue-800 dark:text-blue-200">
              ...
              <ol className="list-decimal ml-4 space-y-1">
                ...
                <li>Fill in your inward stock data</li>
...
export default BulkInwardUpload;
