            ...
            <>
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(260px,1fr))',gap:'24px',marginBottom:'40px'}}>
                {[
                  ...
                  {title:'Inventory Value',value:(()=>{const total=realProducts.reduce((sum,p)=>sum+((parseFloat(p.costPrice)||parseFloat(p.mrp)||parseFloat(p.price)||0)*(parseFloat(p.quantity)||0)),0);return total>=100000?'₹'+(total/100000).toFixed(2)+'L':total>=1000?'₹'+(total/1000).toFixed(2)+'K':'₹'+total.toFixed(0);})(),change:'+0%',icon:'💰',color:'#8b5cf6',gradient:'linear-gradient(135deg, #8b5cf6, #7c3aed)',trend:'up',page:'reports',view:''},
                  {title:'Low Stock SKU',value:realProducts.filter(p=>(parseFloat(p.quantity)||0)<(parseFloat(p.lowStockThreshold)||parseFloat(p.minThreshold)||10)).length.toString()+' SKUs',change:'-0%',icon:'⚠️',color:'#ef4444',gradient:'linear-gradient(135deg, #ef4444, #dc2626)',trend:'down',page:'reports',view:'low-stock-list'}
                ].map((stat,i)=>(
